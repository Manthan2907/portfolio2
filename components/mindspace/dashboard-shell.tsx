...
import { BarChart3, Check, ChevronLeft, ChevronRight, Download, HeartPulse, Home, LogOut, Plus, Settings, Sparkles, Trash2, X } from 'lucide-react'
import { toast } from 'sonner'
import { useAuthStore } from '@/store/authStore'
import { authService } from '@/lib/authService'
import { emptyWellnessData, loadWellnessData, saveWellnessData, type WellnessData, type Habit, newHabit, daysInMonth, dateKey, habitMetrics } from '@/lib/wellness'
...
export function DashboardShell({ section = 'overview' }: { section?: 'overview' | 'habits' | 'insights' | 'settings' }) {
  ...
  useEffect(() => { if (user) setData(loadWellnessData(user.uid)) }, [user])
  useEffect(() => { if (user) saveWellnessData(user.uid, data) }, [data, user])
